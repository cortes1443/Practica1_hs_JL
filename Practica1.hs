...
guardarUniversidad universidad = do
    resultado <- reintentar 5 (withFile "universidad.txt" WriteMode (\h ->
        mapM_ (hPutStrLn h . mostrarEstudiante) universidad))
    case resultado of
...
cargarUniversidad = do
    ...
    case resultado of
        Left ex -> do
            ...
            return []
        Right contenido -> return (mapMaybe parsearEstudiante (lines contenido))

-- Función para mostrar la información de un estudiante en formato CSV
mostrarEstudiante :: Estudiante -> String
mostrarEstudiante estudiante =
    idEstudiante estudiante ++ "," ++ show (entrada estudiante) ++ "," ++ maybe "Nothing" show (salida estudiante)

-- Función para convertir una línea de texto en un estudiante
parsearEstudiante :: String -> Maybe Estudiante
parsearEstudiante linea = case words linea of
    [id, entradaStr, salidaStr] -> do
        entrada <- readMaybe entradaStr
        let salida = readMaybe salidaStr
        Just $ Estudiante id entrada salida
    _ -> Nothing
        ...
        "3" -> do
            ...
            case buscarEstudiante idEstudiante universidad of
                Just estudiante -> do
                    ...
                    putStrLn $ "El estudiante con ID " ++ idEstudiante ++ " ha estado en la universidad por " ++ show tiempoTotal ++ " segundos."
